<template>
  <div>
    <h2 id="tech-stack">
      <NuxtLink to="#tech-stack">
        {{ $t('techStack') }}
      </NuxtLink>
    </h2>
    <div class="flex flex-wrap gap-4">
      <TechStackIcon v-for="tech in techs" :key="tech.name" :tech="tech" />
    </div>
  </div>
</template>
...
const techList: Record<string, Tech> = {
  ...
  tauri: {
    ...
    icon: 'i-vscode-icons:file-type-tauri',
  },
  json: {
    name: 'JSON',
  ...
  vercel: {
    ...
    icon: 'i-vscode-icons:file-type-vercel',
  },
  figma: {
    name: 'Figma',
